...
abstract class BaseUser extends BaseObject  implements Persistent {
	 ...
	 * related UserCredentials from storage. If this User is new, it will return
		...
		if ($this->collUserCredentials === null) {
			if ($this->isNew()) {
			   $this->collUserCredentials = array();
				...
				$criteria->add(UserCredentialPeer::USER_ID, $this->id);
	 ...
	 */
	public function getUserCredentialsJoinCredential($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(UserPeer::DATABASE_NAME);
		}
		...
		if ($this->collUserCredentials === null) {
			if ($this->isNew()) {
				...
				$this->collUserCredentials = UserCredentialPeer::doSelectJoinCredential($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
